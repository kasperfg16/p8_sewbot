...
class MJ_Controller(object):
    ...
    def __init__(self, model=None, simulation=None, Renderer=None):
        ...
        self.Renderer = mujoco.Renderer(self.model) if Renderer is None else Renderer
    ...
    def create_lists(self):
        ...
        for i in range(len(self.data.ctrl)):
            ...
            item.append(mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR , self.model.actuator_trnid[i][0]))
    ...
    def move_ee(self, ee_position, **kwargs):
        ...
        Args:
            ee_position: List of XYZ-coordinates of the end-effector (EE for UR5 setup).
    ...
    def ik(self, ee_position):
        ...
        try:
            ...
            ee_position_base = (
                ee_position - self.data.geom_xpos[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, "base_link")]
            )

            print('ee_position_base', ee_position_base)

            # By adding the appr. distance between EE and grasp center, we can now specify a world target position
            # for the grasp center instead of the EE
            ...
            print('gripper_center_position', gripper_center_position)

            joint_angles = self.ee_chain.inverse_kinematics(
                gripper_center_position, [0, 0, -1], orientation_mode="X"
            )

            print('joint_angles_2', joint_angles)

            prediction = (
                self.ee_chain.forward_kinematics(joint_angles)[:3, 3]
                + self.data.geom_xpos[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, "base_link")]
                - [0, -0.005, 0.16]
            )
    ...
    def ik_2(self, pose_target):
        ...
        target_position -= self.data.geom_xpos[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, "base_link")]
